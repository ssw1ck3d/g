...
class XFrogApp extends App {
    ...
    async startup() {
        ...
        let buf = [];
        await anura.x86.openpty('while true; do DISPLAY=:0 xdotool search --maxdepth 1 ".*" 2>/dev/null; echo EOF;sleep 2; done', 0, 0, (data) => {
    ...
    async spawn_xwindow(xwid) {
        ...
        const sfocus = async () => {
            ...
            timeout = setTimeout(() => {
                ...
                if (isNew)
                    anura.x86.openpty(`DISPLAY=:0 xdotool search --maxdepth 1 --onlyvisible ".*" 2>/dev/null | while read wid; do DISPLAY=:0 xdotool windowunmap $wid; done; DISPLAY=:0 xdotool windowmap ${xwid}; DISPLAY=:0 xdotool windowmove ${xwid} 0 0; DISPLAY=:0 xdotool windowsize ${xwid} ${win.width} ${win.height - 28}`, 0, 0, console.log);
                setTimeout(() => {
        ...
        await anura.x86?.openpty(`DISPLAY=:0 xdotool getwindowgeometry ${xwid} 2>/dev/null | grep -o '[0-9]*x[0-9]*'; while true; do DISPLAY=:0 xdotool getwindowname ${xwid}; sleep 5; done`, 0, 0, (data) => {
            if (!win) {
