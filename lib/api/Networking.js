            ...
            if (this.loopback.addressMap.has(port))
                ...
                if (anura.x86?.termready) {
                    return await new Promise((resolve) => {
                        ...
                        const pty = anura.x86.openpty(`curl -o - -s -i -X ${requestObj.method} http://localhost:${urlObj.port}${urlObj.pathname}${urlObj.search} | cat | base64 && echo -n ${endMarker}`, 0, 0, async (data) => {
                            try {
                                buffer += data;
